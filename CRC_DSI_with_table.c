#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdint.h>
#include <windows.h>

int32_t main()
{
    uint8_t input_command[10];
    uint32_t input_command_int;
    uint8_t input_command_binary[32] = {};
    int32_t set_position = 0;
    int32_t CRC;
    int32_t CRC_table_command_1[] =
    {
    
    };
    int32_t CRC_table_command_0[] =
    {
    
    };
    while(1){
        printf("input command(Hex) : ");
        scanf("%x",&input_command_int);

        CRC = 0b11111111;
        input_command_int = input_command_int << 8;

        for (register int32_t i = 0; i < 32; ++i)
        {
            input_command_binary[i] = (input_command_int & (1 << (31 - i))) >> (31 - i);
        }

        for (set_position = 0; set_position < 32; set_position ++)
        {
            CRC = (input_command_binary[set_position] == 1)? CRC_table_command_1[CRC] : CRC_table_command_0[CRC] ;
        }

        printf("CRC : %02X\n", CRC);
    }
    return 0;
}
